...
locals {
  dynamodb = {
    # [SH.DynamoDB.1] DynamoDB tables should automatically scale capacity with demand
    "XQGJDGQQKGCR" = {
    ...
    # [SH.DynamoDB.2] DynamoDB tables should have point-in-time recovery enabled
    "ITCPEFZTGHOG" = {
      "${local.all_ous["Infrastructure"].id}" = local.all_ous["Infrastructure"].arn,
      "${local.all_ous["Sandbox"].id}"        = local.all_ous["Sandbox"].arn,
      "${local.all_ous["Workloads"].id}"      = local.all_ous["Workloads"].arn
...
resource "aws_controltower_control" "dynamodb" {
  for_each = merge([for control, ou_map in local.dynamodb :
