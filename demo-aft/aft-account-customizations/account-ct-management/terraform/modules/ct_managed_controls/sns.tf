...
locals {
  sns = {
    # # Has been deprecated
    # # [SH.SNS.1] SNS topics should be encrypted at-rest using AWS KMS
    # "DXZDOLIUCILU" = {
    ...
    # # Has been deprecated
    # # [SH.SNS.2] Logging of delivery status should be enabled for notification messages sent to a topic
    # "UJGAATDFQPEB" = {
    #   "${local.all_ous["Infrastructure"].id}" = local.all_ous["Infrastructure"].arn,
    #   "${local.all_ous["Sandbox"].id}"        = local.all_ous["Sandbox"].arn,
    #   "${local.all_ous["Workloads"].id}"      = local.all_ous["Workloads"].arn
...
resource "aws_controltower_control" "sns" {
  for_each = merge([for control, ou_map in local.sns :
