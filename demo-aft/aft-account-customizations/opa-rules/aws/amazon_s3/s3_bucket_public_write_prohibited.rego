...
title := "S3_BUCKET_PUBLIC_WRITE_PROHIBITED"
...
is_present(res, bucket_addr) if {
	...
	res.expressions.acl.constant_value == "public-read-write"
}
...
violations contains response if {
	...
	object.get(terraform.resources[j].values, "acl", "private") == "public-read-write"

	response := terraform.ocsf_response(id, title, {
		"message": sprintf(
			"ID %s %s: Resource (%s) is an S3 bucket that does not have acl set to public-read-write. Please set acl as described in the documentation https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/s3_bucket",
...
violations contains response if {
	...
	response := terraform.ocsf_response(id, title, {
		"message": {
			...
			"OWNER": owner,
			"SEVERITY": level,
			"DESCRIPTION": "Resource is an S3 bucket that does not have aws_s3_bucket_acl set to public-read-write. Please set acl as described in the documentation https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/s3_bucket",
