## Create EC2 KeyPair for access using personal SSH Public Key
resource "aws_key_pair" "linuxjump" {
  key_name   = "${var.namespace}_linuxjump_keypair"
  public_key = "ssh-rsa  mlynch1985@gmail.com"
}

## Create EC2 Launch Template
resource "aws_launch_template" "linuxjump" {
  name                   = "${var.namespace}_linuxjump_launchtemplate"
  image_id               = data.aws_ami.amazon_linux_2.id
  instance_type          = "t2.medium"
  key_name               = aws_key_pair.linuxjump.key_name
  vpc_security_group_ids = ["${data.aws_security_group.allow_vpc_ssh.id}", "${data.aws_security_group.allow_office_ssh.id}"]
  user_data              = filebase64("${path.module}/userdata.sh")
  iam_instance_profile {
    name = aws_iam_instance_profile.linuxjump.name
  }
}

## Create AutoScalingGroup
resource "aws_autoscaling_group" "linuxjump" {
  name                 = "${var.namespace}_linuxjump_autoscalinggroup"
  max_size             = 5
  min_size             = 1
  desired_capacity     = 1
  force_delete         = true
  vpc_zone_identifier  = data.aws_subnet_ids.subnets_private.ids
  target_group_arns    = [aws_lb_target_group.linuxjump.id]
  termination_policies = ["OldestLaunchConfiguration", "OldestInstance"]
  launch_template {
    id      = aws_launch_template.linuxjump.id
    version = "$Latest"
  }
  lifecycle {
    create_before_destroy = true
  }

  tag {
    key                 = "Name"
    value               = "${var.namespace}_linuxjump"
    propagate_at_launch = true
  }
  tag {
    key                 = "namespace"
    value               = var.namespace
    propagate_at_launch = true
  }
  tag {
    key                 = "environment"
    value               = var.environment
    propagate_at_launch = true
  }
  timeouts {
    delete = "15m"
  }
}
