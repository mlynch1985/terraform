...
module "sechub_archive_bucket" {
  ...
  replication_configuration = {
    role = aws_iam_role.sechub_archive_bucket_replication.arn
    ...
    rules = [
      {
        ...
        destination = {
          bucket             = module.sechub_archive_replica_bucket.arn
          storage_class      = "INTELLIGENT_TIERING"
          replica_kms_key_id = module.sechub_archive_kms_key_replica_region2.arn
...
module "sechub_archive_replica_bucket" {
  source = "../../modules/s3_bucket"
  #checkov:skip=CKV_AWS_300:Lifecycle rule is set but checkov is producing false positive
  ...
  bucket_name                      = replace(local.stack_replica_name, "_", "-")
  bucket_policy                    = data.aws_iam_policy_document.sechub_archive_replica_bucket_policy.json
  key_arn                          = module.sechub_archive_kms_key_replica_region2.arn
  ...
  # S3-7  - Skipping OPA check for s3 bucket server side encryption as it is a false positive
  ...
  # S3-12 - ???
  # S3-14 - Skipping OPA check for s3 default kms encryption as it is a false positive
  # S3-15 - Skipping OPA check for Object Lock s3 Buckets which doesn't require object lock configuration
  tags = {
    "opa_skip" = "S3-2/S3-7/S3-9/S3-10/S3-12/S3-14/S3-15"
  }
...
module "sechub_archive_kms_key_replica_region2" {
  source = "../../modules/kms_key_replica"

  key_name        = "aft/${local.stack_name}"
  key_policy      = data.aws_iam_policy_document.sechub_archive_kms_key.json
  primary_key_arn = module.sechub_archive_kms_key_region1.arn
...
module "sechub_archive_kms_key_replica_region3" {
  source = "../../modules/kms_key_replica"
...
module "sechub_archive_replica_access_logs_kms_key" {
  source = "../../modules/kms_key"

  key_name   = "aft/${local.stack_name}_replica_access_logs"
  key_policy = data.aws_iam_policy_document.sechub_archive_access_logs_kms_key.json
...
data "aws_iam_policy_document" "sechub_archive_bucket_policy" {
  ...
  statement {
    ...
    effect    = "Deny"
    actions   = ["*"]
    resources = [module.sechub_archive_bucket.arn, "${module.sechub_archive_bucket.arn}/*"]
    principals {
      type        = "*"
      identifiers = ["*"]
    }
...
data "aws_iam_policy_document" "sechub_archive_replica_bucket_policy" {
  #checkov:skip=CKV_AWS_111:Condition is restricting to accounts only within the organizations.
  # allows cross account access from org principals only using a known firehose role name
  ...
  statement {
    ...
    effect    = "Deny"
    actions   = ["s3:PutObject"]
    resources = ["${module.sechub_archive_replica_bucket.arn}/*"]
    principals {
      type        = "*"
      identifiers = ["*"]
    }
...
resource "aws_iam_role" "sechub_archive_bucket_replication" {
  name = "s3-bucket-replication-${local.stack_name}"
  path = "/aft/"
...
data "aws_iam_policy_document" "sechub_archive_bucket_replication" {
  statement {
  ...
  statement {
    ...
    condition {
      test     = "StringLikeIfExists"
      variable = "s3:x-amz-server-side-encryption-aws-kms-key-id"
      values   = [module.sechub_archive_kms_key_replica_region2.arn]
    }
    resources = ["${module.sechub_archive_replica_bucket.arn}/*"]
  }
  statement {
    ...
    condition {
      ...
      values   = ["s3.${module.sechub_archive_bucket.region}.amazonaws.com"]
    }
    condition {
      test     = "StringLike"
  ...
  statement {
    ...
    condition {
      test     = "StringLike"
      variable = "kms:ViaService"
      values   = ["s3.${module.sechub_archive_replica_bucket.region}.amazonaws.com"]
    }
    condition {
      test     = "StringLike"
      variable = "kms:EncryptionContext:aws:s3:arn"
      values   = ["${module.sechub_archive_replica_bucket.arn}/*"]
    }
    resources = [module.sechub_archive_kms_key_replica_region2.arn]
  }
}

resource "aws_iam_policy" "sechub_archive_bucket_replication" {
  name   = "s3-bucket-replication-${local.stack_name}"
  policy = data.aws_iam_policy_document.sechub_archive_bucket_replication.json
...
resource "aws_iam_policy_attachment" "sechub_archive_bucket_replication" {
  name       = "s3-bucket-replication-${local.stack_name}"
  roles      = [aws_iam_role.sechub_archive_bucket_replication.name]
  policy_arn = aws_iam_policy.sechub_archive_bucket_replication.arn
}
...
data "aws_iam_policy_document" "sechub_archive_kms_key" {
  ...
  statement {
    ...
    condition {
      ...
      values = [
        module.firehose_role.arn,
        aws_iam_role.sechub_archive_bucket_replication.arn
      ]
    }
  }
  statement {
